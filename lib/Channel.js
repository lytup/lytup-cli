...
Channel.prototype.upload = function(files) {
  ...
  request.post(CFG.apiUri + "/channels", {
    ...
    self.progress = new Progress(channel.progress);
...
Channel.prototype.download = function(uri) {
  ...
  request.get(CFG.apiUri + "/channels/" + channelId, {
    ...
    req.on("response", function(res) {
      ...
      var info = util.format("receiving %s of file(s) - saving to \"%s\"",
        UTIL.humanizeBytes(self.progress.total), filename.cyan);
      self.progress.start(info);
