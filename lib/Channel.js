...
Channel.prototype.upload = function(files) {
  ...
  request.post(Config.API_URI + "/channels", {
    ...
    console.log("Receive files(s) " + lytup.cyan);
    ...
    var ws = new WebSocket(Config.WS_URI + "?" + qs.stringify({
      channel: self.id,
      role: global.role
    }));
      ...
      switch (msg.event) {
        case "UPLOAD":
          ...
          var info = sprintf("Sending %s of file(s)",
            Utility.humanizeBytes(self.progress.total));
  ...
  request.get(Config.API_URI + "/channels/" + channelId, {
    ...
    req.on("response", function(res) {
      ...
      var info = sprintf("Receiving %s of file(s) %s saving to \"%s\"",
        Utility.humanizeBytes(self.progress.total), "|".grey, self.downloadFile.name.cyan);
      console.log(info.green);
